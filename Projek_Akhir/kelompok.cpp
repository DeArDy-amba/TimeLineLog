...
void tambahBuku() {
    ...
    b.isbn = buatISBN(count + 1, b.penerbit);
...
void peminjaman_blm_dikembalikan() {
    ...
    while (getline(inFile, line)) {
        if (line.find("Judul_Buku:") == 0)
            ...
            int i;
            if (status != "1") continue; // hanya proses jika status peminjaman adalah 1 (belum dikembalikan)
